#include<iostream>
#include<vector>
#include<cstring>
#include<queue>
#include<algorithm>
#include<fstream>
using namespace std;
const int N=1e5+5;
#define pb push_back
vector<int> val;
vector<string> string_list,compress_string;
string Line;
int get_id(char ch){
    if(ch>='A'&&ch<='Z') return ch-'A'+1;
    if(ch>='a'&&ch<='z') return ch-'a'+27;
    if(ch>='0'&&ch<='9') return ch-'0'+53;
    if(ch=='_') return 63;
    return 64;
}
struct Trie{
    struct node{
        int fail,to[129],ans;
    }tr[N];
    int tot,in[N];
    void clear(){
        for(int i=0;i<=tot;i++){
            memset(tr[i].to,0,sizeof(tr[i].to));
            tr[i].fail=0;
            tr[i].ans=0;
            in[i]=0;
        }
        tot=0;
    }
    bool insert(int start,int end){
        bool flag=0;
        int l=end-start+1;
        int u=0;
        for(int i=start;i<=end;i++){
            if(!tr[u].to[get_id(Line[i])]){
                tr[u].to[get_id(Line[i])]=++tot;
                flag=1;
            }
            u=tr[u].to[get_id(Line[i])];
        }
        return flag;
    }
}T;
long long compress_value;
struct Suffix_Automaton{
    int to[N*2][129],fa[N*2],l[N*2];//fa 和 lnk 是一个东西
    int tot;
    int cnt[N*2];
    const int inf=2*N-1;
    pair<int,int> index[N*2];
 
    Suffix_Automaton(){
        fa[0]=-1;
    }
    void clear(){
        for(int i=0;i<=tot;i++){
            memset(to[i],0,sizeof(to[i]));
            fa[i]=0;
            l[i]=0;
            cnt[i]=0;
            index[i]={0,0};
        }
        fa[0]=-1;
        tot=0;
    }
    int add(int c,int last,pair<int,int> position){
        if(to[last][c] && l[to[last][c]]==l[last]+1){
            cnt[to[last][c]]++;
            return to[last][c];
        }
        int p=last,np=++tot,nq;
        bool flag=0;
        l[np]=l[p]+1;cnt[np]++;
        index[np]=position;
        while(p!=-1&&!to[p][c]){
            to[p][c]=np;
            p=fa[p];
        }
        if(p==-1){
            fa[np]=0;
            return np;
        }
        else{
            int q=to[p][c];
            if(l[q]==l[p]+1){
                fa[np]=q;
                return np;
            }
            else{
                if(p==last){
                    flag=1,np=inf,tot--;
                }
                nq=++tot;
                l[nq]=l[p]+1;
                index[nq]=index[q];
                for(int i=0;i<129;i++) to[nq][i]=to[q][i];
                fa[nq]=fa[q];fa[np]=fa[q]=nq;
                while(p!=-1&&to[p][c]==q){
                    to[p][c]=nq;
                    p=fa[p];
                }
            }
        }
        return flag?nq:np;
    }
 
    vector<int> vec[N];
    void topu(){
        int maxl=0;
        for(int i=1;i<=tot;i++){
            vec[l[i]].pb(i);
            maxl=max(maxl,l[i]);
        }
        for(int i=maxl;i>=1;i--){
            for(auto u:vec[i]){
                cnt[fa[u]]+=cnt[u];
            }
            vec[i].clear();
        }
    }
    bool solve(){
        int maxvalue=0,maxvalue_id=-1;
        for(int i=1;i<=tot;i++){
            if(cnt[i]==1 || l[i]==1) continue;
            int value=(l[i]-1)*(cnt[i]-1);
            if(value>maxvalue){
                // printf("l[%d]=%d, cnt[%d]=%d, value=%d\n",i,l[i],i,cnt[i],value);
                maxvalue=value;
                maxvalue_id=i;
            }
        }
        // printf("maxvalue=%d, maxvalue_id=%d\n",maxvalue,maxvalue_id);
        if(maxvalue_id==-1) return 0;
        int j=index[maxvalue_id].first,k=index[maxvalue_id].second-l[maxvalue_id]+1;
        // printf("j=%d, k=%d\n",j,k);
        string tmp(string_list[j].begin()+k,string_list[j].begin()+k+l[maxvalue_id]);
        compress_string.push_back(tmp);
        compress_value+=maxvalue;
        return 1;
    }
}SAM;
int pos[N],border[N];
string kmp(string a,string b,int ch){//a 为原串（长），b 为需要被匹配的串（短）
    string res;
    int la=a.size(),lb=b.size();
    int j=0;
    for(int i=1;i<lb;i++){
        while(j&&b[i]!=b[j]) j=border[j];
        if(b[i]==b[j]) j++;
        border[i+1]=j; 
    }
    j=0;
    for(int i=0;i<la;i++){
        res.push_back(a[i]);
        while(j&&b[j]!=a[i]) j=border[j];
        if(b[j]==a[i]) j++;
        if(j==lb){
            res.erase(res.end()-lb,res.end());
            res.push_back(ch);
            j=0;
        }
    }
    return res;
}
void Compress(int k){
    // printf("D\n");
    SAM.clear();
    // printf("当前的字符串列表：\n");
    for(int i=0;i<string_list.size();i++){
        // vector<int> s=string_list[i];
        // for(auto x:s) printf("%d ",x);
        // printf("\n");
        string s=string_list[i];
        for(int j=0;j<s.size();j++){
            if(j==0) pos[j]=SAM.add(s[j],0,{i,j});
            else pos[j]=SAM.add(s[j],pos[j-1],{i,j});
        }
    }
    
    SAM.topu();


    if(!SAM.solve()) return;


    string tmp=compress_string.back();
    for(int i=0;i<string_list.size();i++){
        string_list[i]=kmp(string_list[i],tmp,63+k);
    }



    // printf("被压缩的字符串：\n");
    // for(auto x:compress_string){
    //     for(auto y:x) printf("%d ",y);
    //     printf("\n");
    // }
    // printf("-------------\n");
}
void Read_data(){
    int len=Line.length();
    int start=-1;
    for(int i=0;i<len;i++){
        if(Line[i]=='\"'){
            i++;
            int start=i;
            for(;i<len;i++){
                if(Line[i]=='\"') break;
            }
            if(T.insert(start,i-1)){
                string tmp;
                for(int j=start;j<=i;j++) tmp.push_back(get_id(Line[j]));
                string_list.push_back(tmp);
            }
        }
        else if(Line[i]==':'){
            long long tmp_val=0;
            bool flag=1;
            i+=2;
            for(;i<len;i++){
                if(Line[i]==',') break;
                if(Line[i]=='-') flag=0;
                else tmp_val=tmp_val*10+(Line[i]^48);
            }
            tmp_val*=(flag)?1:-1;
            val.push_back(tmp_val);
        }
    }
}
int main(){
    // freopen("dataset0000.txt","r",stdin);
    ios::sync_with_stdio(false);
    cin.tie(nullptr);
    cout.tie(nullptr);
    clock_t start=clock();
    ifstream file("dataset0000.txt");

    while(1){
        int file_num=0;
        string_list.clear();compress_string.clear();val.clear();
        T.clear();
        while (getline(file, Line) && file_num<1000) {
            Read_data();
            file_num++;
        }
        // printf("读取%d行数据\n",file_num);
        for(int i=1;i<=32;i++){
            Compress(i);
            // printf("Compress %d\n",i);
        }    
        if(file_num!=1000) break;
        // printf("Done\n");
    }
    
    // while (getline(file, Line)) {
    //     Read_data();
    // }
    // for(int i=1;i<=1;i++){
    //     getline(file,Line);
    //     Read_data();
    //     printf("读取第%d行数据\n",i);
    // }
    // for(int i=1;i<=200;i++){
    //     Compress(i);
    //     // printf("Compress %d\n",i);
    // }    

   
    // for(auto x:compress_string){
    //     printf("%s\n",x.c_str());
    // }
    // printf("------------\n");
    printf("接受\n");
    printf("压缩效益：%d\n",compress_value);
    printf("Time: %lf\n",(double)(clock()-start)/CLOCKS_PER_SEC);
    return 0;
}